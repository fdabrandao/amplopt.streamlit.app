...
def solve(input_data: dict[str, Any], duration: int, provider: str) -> dict[str, Any]:
    ...
    ampl.param["fixed_cost"] = pd.read_json(
        io.StringIO(input_data["fixed_cost"]), orient="table"
    )
    ampl.param["facility_capacity"] = pd.read_json(
        io.StringIO(input_data["facility_capacity"]), orient="table"
    )
    ampl.param["variable_cost"] = pd.read_json(
        io.StringIO(input_data["variable_cost"]), orient="table"
    )
    ampl.param["customer_demand"] = pd.read_json(
        io.StringIO(input_data["customer_demand"]), orient="table"
    )
    ...
    solve_output = ampl.get_output("include floc_bend.run;")
    ...
    return {
        "solutions": [
            {
                "facility_open": ampl.get_data("facility_open")
                .to_pandas()
